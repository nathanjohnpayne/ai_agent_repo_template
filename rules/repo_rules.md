...
- check_coderabbit_config: `.coderabbit.yml` must exist at repo root, parse as YAML, and set `reviews.profile: chill` — `assertive` re-enables 🧹 Nitpick comments per CodeRabbit's profile semantics. See #237 for the rationale (the 2026-05-13 sweep surfaced 56 unresolved nit threads across 9 repos, none substantive). Requires `yq` (mikefarah/yq v4+) on the runner.
