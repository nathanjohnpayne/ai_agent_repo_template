...
#   - specs/$repo_name.md             (PRD/spec stub)
...
bootstrap::_write_placeholder_scaffolds() {
  ...
  # The PRD/spec stub. Single-line content per #207's spec. We use a
  # heredoc through `tee` so bootstrap::run captures the side effect
  # cleanly (and so dry-run prints the command line).
  local spec_body="TODO: write the PRD/spec for this repo. Wizard left this empty per #156 deliberate-not-in-scope decision."
...
"
        ...
        done_lines="$done_lines  [done] Empty PRD / spec / plan scaffolds written
"
        ;;
    esac
  done

  {
    ...
    echo "  2. Write the PRD / spec for this repo:"
